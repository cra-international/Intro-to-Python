...
#In Python all six normal comparison operators are defined
...
#The 'and' operator checks if a condition is True and second condition is True
...
expession2 = False or False and True or False #Evaluates to False
...
#Note that in the order of Python operations, comparison operators are 
...
if q > p:
    ...
    print("k is less than or equal to j")
...
#Note: Conditional statements like 'if' and 'else' are always followed by colons

#You may notice in the above statements that the statements following the
#'if' and 'else' statements are all indented. This is in order to
#differentiate the code that is executed only if the conditional is True from
#the rest of the code in your python file. We will see this structure more
#when we talk about function definitions.

#What if we have more than one condition we would want to test? Say, if we to
...
#Now you try! Use Python to commpare the length of the strings below.
