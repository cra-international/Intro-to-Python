...
#by n is the same as adding the same list to itself n-1 times
...
#in the draft, the the name of the player and the player's school all in 
#the print statement

for i in range(10):
    name = top_10_players[i]
    school = college_teams[name]
    print("Selected with the number " + str(i + 1) + " pick in the NFL Draft is " + name + " from " + school)
...
#or calculating a statistic for each year from 2000 to 2019 based on data
...
while not stop:
    print("Ring!")
    user_input = input("Would you like to stop the alarm?")
    if user_input == "Yes":
        stop = True
